# Ellevux — Vault Index (MOC)

The central map of content for the **Ellevux** AI receptionist project. Everything links back here so the graph stays connected.

## Project Docs
- [[claude]] — context & directives for Claude (architecture, design rules, tone)
- [[handover]] — project handover (structure, recent work, notes for future dev)

## Pages
- [[react_landing.html]] — main landing page
- [[dashboard.html]] — user dashboard
- [[login.html]] — login flow
- [[signup.html]] — signup flow
- [[pricing.html]] — subscription tiers
- [[demo.html]] — interactive audio demo
- [[docs.html]] — technical documentation
- [[changelog.html]] — product updates

## Backend & Assets
- [[supabase-config.js]] — Supabase backend configuration
- [[logo.png]] — primary branding asset
- [[demo.mp3]] — audio sample used by the demo page
